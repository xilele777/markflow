...
set -Eeuo pipefail
umask 077
ROOT=$(realpath -e -- "${MARKFLOW_DEPLOY_ROOT:-/srv/markflow}")
...
if [[ -z "${MARKFLOW_PUBLIC_BASE_URL:-}" && -r "$ROOT/shared/public-url" ]]; then
  MARKFLOW_PUBLIC_BASE_URL=$(cat -- "$ROOT/shared/public-url")
fi
...
pm() { env -i HOME="$HOME" PATH="$PATH" USER="${USER:-markflow}" LANG=C.UTF-8 PM2_HOME="${PM2_HOME:-$HOME/.pm2}" pm2 "$@"; }
atomic_link() {
  ln -s -- "$1" "$ROOT/.current-$$"
  mv -Tf -- "$ROOT/.current-$$" "$ROOT/current"
}
start_release() {
  pm delete markflow >/dev/null 2>&1 || true
  pm start "$1/ecosystem.config.cjs" --only markflow
}
check_release() {
  local expected
  expected=$(basename "$(readlink -f -- "$ROOT/current")")
  MARKFLOW_EXPECTED_RELEASE="$expected" MARKFLOW_SMOKE_ENV_FILE="$ROOT/shared/smoke.env" bash "$DIR/scripts/smoke.sh" "${MARKFLOW_SMOKE_BASE_URL:-http://127.0.0.1:8080}" --api-only &&
  MARKFLOW_EXPECTED_RELEASE="$expected" MARKFLOW_SMOKE_ENV_FILE="$ROOT/shared/smoke.env" bash "$DIR/scripts/smoke.sh" "${MARKFLOW_PUBLIC_BASE_URL:?Set MARKFLOW_PUBLIC_BASE_URL to the candidate Nginx HTTPS URL}"
}
SWITCHED=0
rollback() {
  local code=$?
...
trap rollback ERR
trap 'false' INT TERM
: "${MARKFLOW_PUBLIC_BASE_URL:?Set candidate Nginx URL before activation}"
