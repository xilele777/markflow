...
首轮推送 `3b7dd44` 的 [CI](https://github.com/xilele777/markflow/actions/runs/36015519551) 中，静态检查、前端测试 / 构建、发布打包和 Linux 部署脚本夹具均通过；后端集成任务在准备 MinIO 时遇到官方 registry 401，尚未执行测试。官方 `dl.min.io` 同时返回 410，并说明社区项目已归档且不再提供二进制文件。

本地缓存镜像不受影响，但干净环境无法复验。因此增加 `deploy/Dockerfile.minio`：从官方 `minio/minio` 提交 `07c3a429bfed433e49018cb0f78a52145d4bedeb`（2025-09-07 版本）和 `minio/mc` 提交 `7394ce0dd2a80935aded936b09fa12cbb3cb8096`（2025-08-13 版本）构建，保留许可证文件。Compose 与 CI 共用该构建定义，CI 缓存构建层；不引入第三方镜像源，不修改现有数据卷。这一方案用于开发 / 测试复现，生产存储选型仍属单独上线范围。
