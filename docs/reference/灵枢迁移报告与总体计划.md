...
> 所有事实均来自对 `F:/label/submission/`（灵枢两仓）与 `F:/label/LabelHub/` 源码、配置、git 历史的直接读取，关键处标注了文件路径。
...
2. 前提与授权
3. 迁移范围定义（三张清单）
4. 功能差距矩阵：LabelHub → 灵枢
5. 工程差距矩阵：灵枢当前状态 vs 目标状态
6. 目标架构与仓库布局
7. 关键技术决策（ADR）
8. 分阶段路线图与工作量
9. 风险登记册
...
14. 附录 C：本机环境检查结果
15. 附录 D：迁移中发现的灵枢隐藏问题

---

## 1. 决策摘要

**决策：以灵枢（LingShu）为基座，把 LabelHub 的工程能力与少量业务功能移植到灵枢上；LabelHub 转入维护状态，不再新增功能。**

理由（详见前一份评估报告 §7）：

- 迁移成本不对称。把灵枢的"多池派发引擎 + 数据资产层 + MQ + 多租户"搬进 LabelHub 等于重写 LabelHub 后端核心；把 LabelHub 的 CI/CD、安全加固、测试、性能治理搬进灵枢是纯加法，不动灵枢架构。
- 灵枢的业务模型天花板远高于 LabelHub（AI 原生、流程可编排、流式接入、多租户）。
...
| # | 目标 | 当前状态 | 衡量标准 |
|---|---|---|---|
| G1 | 灵枢**可复现**：任何人 clone 后 30 分钟内本地跑通完整流水线 | 无 DDL、无容器、依赖手工 SSH 隧道与火山 TOS 账号 | `docker compose up` + `./mvnw verify` 全绿 + 冒烟脚本通过 |
| G2 | 灵枢**安全可上线** | 口令硬编码、CORS 全开带凭据、登录零限流、apiKey 明文 | 附录 D 的 P0 全部关闭 |
| G3 | 灵枢**有护栏**：CI 门禁 + 测试覆盖前后端 | 两仓库均无 CI；前端 0 测试 | 前后端 CI 绿灯为合并前提；前端关键模块有测试 |
| G4 | 灵枢**可运维**：指标、健康检查、失败可见、消息不丢 | 无 actuator、AI 失败静默、MQ 发送失败仅记日志 | Prometheus 指标可采、AI 失败落库可查、MQ 发送失败进 outbox 重试 |
| G5 | 灵枢**前端体验与性能治理**对齐 LabelHub | 单 JS 1.82 MB、无预算、无监控 | 入口 gzip 预算进 CI；web-vitals 上报 |
| G6 | 移植 LabelHub 有价值的业务功能（可选） | 站内通知、截止提醒、离线提示等灵枢均无 | 按 §8 阶段 4 逐项验收 |

---

## 2. 前提与授权

1. **代码使用授权。** 灵枢两仓库（`github.com/onlyactwo/LingShu`、`github.com/onlyactwo/lingshu-web`）**均无 LICENSE 文件**，作者为 Li ZhuoXi（onlyactwo）。默认"保留所有权利"。在开始迁移前必须取得作者书面同意（或确认其为团队共同交付物），并在新仓库 README 中保留原作者署名与来源链接。这是硬前提，不满足则停止。
2. **服务器资源。** LabelHub 现有 ECS 为 1.6 GB 内存、且拉不到 docker.io 镜像（`LabelHub/DEPLOY.md` §九）。灵枢全栈内存需求见 §9 风险 R1。上线前需确认换用 ≥ 4 GB 的机器或改用系统级安装中间件。
3. **火山 TOS 账号。** 不是必须的：计划引入 S3 兼容适配（本地 MinIO）。若最终仍要用 TOS，只需切换 `lingshu.oss.provider=tos` 并在 `sys_config` 写 `tos.config`。
4. **LLM 账号。** 任何 OpenAI 兼容接口（灵枢的 `OpenAiCompatibleLlmClient` 只需 baseUrl/apiKey/model）。

---

## 3. 迁移范围定义（三张清单）

### 3.1 必须修复的灵枢缺陷（不修则不能上线 / 不能复现）

| 编号 | 缺陷 | 证据 | 归属阶段 |
|---|---|---|---|
| F1 | MySQL/Redis 真实口令作为默认值写死在仓库 | `src/main/resources/application.yml`：`${LINGSHU_MYSQL_PASSWORD:lzx2005}` | 阶段 1 |
...
| F15 | 前端单 JS 1.82 MB（gzip 569 KB）、零代码分割 | `lingshu-web-前端/dist/assets/index-*.js` 实测 | 阶段 3 |
...
| P12 | 运维经验（WAL 备份、密钥不进 pm2 dump、fail2ban、非 root 部署账户） | `DEPLOY.md` | 灵枢 `docs/DEPLOY.md`（重写：mysqldump 备份、systemd `EnvironmentFile`、fail2ban、非 root） | 3 |
...
| 内容 | 原因 |
|---|---|
| LabelHub 的 SQLite/PostgreSQL 双数据库层 | 灵枢单 MySQL 更干净 |
| LabelHub 的 Socket.IO 实时通道 | 通知先用轮询；灵枢已有 MQ 骨架，WebSocket 留待有实时协作需求时再加 |
| LabelHub 的模板搭建器（8 种字段） | 灵枢 Puck（18 组件）是超集 |
| LabelHub 的线性状态机与 `crudFactory` | 被灵枢派发引擎与 DDD 分层取代 |
...
对 LabelHub 的 57 个路由端点与 25 个页面逐项对照灵枢的 35 个接口与 20 个页面后的结论：

| LabelHub 功能 | 灵枢对应 | 结论 |
|---|---|---|
| 登录 / 登出 / 当前用户 | `POST /api/auth/login`、`GET /api/user/getCurrentUser` | 已覆盖（灵枢无显式登出，前端清 token 即可） |
| 用户 CRUD + 改密码 | `user/create`、`getUserList`、`changePassword` + JSONL 批量导入 | 已覆盖（灵枢无删除/禁用接口，`status` 字段已预留，阶段 4 可补） |
| 角色：owner / annotator / reviewer | 工作空间 + LABELER / REVIEWER / LABEL_ADMIN + 系统管理员 | 灵枢是超集 |
| 模板管理（可视化搭建 8 种字段） | 标注工具（Puck 18 组件 + IFRAME 外接） | 灵枢是超集 |
| 任务创建 + 分配策略（even_split / manual） | Case 创建 + 5 阶段编排 + FCFS / FIXED_RATIO | 灵枢是超集（无 manual 逐条指派；固定比例可替代） |
| 任务截止时间 / 逾期策略 / 提醒 | 仅有"在手任务超时自动回收" | **缺失**（P14，阶段 4） |
| 任务归档 / 取消归档 | Case 状态 NOT_STARTED / RUNNING / PAUSED / FINISHED（无状态变更接口） | **部分缺失**：灵枢缺"暂停/结束 Case"接口（阶段 4 补 `updateCaseStatus`） |
| 批量导入标注项（JSON，≤ 1000 条） | 数据集 JSONL 上传 + 异步解析（流式、分批） | 灵枢是超集 |
| 标注工作台：草稿自动保存 | `saveTaskResult`（手动保存按钮） | **部分**：灵枢无定时自动保存；阶段 4 在 Puck 渲染器加 debounce 自动保存（小） |
...
| 审核工作台：领取 / 通过 / 驳回 / 重提 | 初检池 / 复检池派发 + 通过 / 驳回打回重标 | 已覆盖（灵枢是"派发"而非"自助领取"；FCFS 策略等价于领取） |
| 审核操作历史 | `round` 字段 + 每轮结果 sample | **部分**（P17，可选） |
| 规则预审 PASS / RISK / FAIL | AI 预审（LLM） | 灵枢更强；规则引擎可作为零成本备选执行器（P16，可选） |
...
| 数据导出（CSV / JSON，前端 Worker） | 异步导出 CSV / JSONL 到对象存储 | 灵枢更强 |

**结论：** 灵枢在业务主链上是 LabelHub 的超集；真正缺失且值得补的是**通知、截止提醒、离线提示、Case 状态控制**四项业务功能，以及全部工程外围。
...
```
F:/label/
├── lingshu-backend/      ← 复制自 submission/LingShu-后端（保留 .git；删除 .idea/、.claude/settings.local.json）
...
└── lingshu-web/          ← 复制自 submission/lingshu-web-前端（保留 .git；删除 *.jsonl 样本）
...
| ADR-9 | **通知先用轮询**（TanStack Query `refetchInterval`） | Socket.IO / SSE | 灵枢无 WebSocket 基础设施；轮询 30 s 对通知场景足够；避免引入 Nginx WebSocket 配置这一 LabelHub 踩过的坑 |
| ADR-10 | **apiKey 用 AES-256-GCM 加密，密钥来自 `LINGSHU_CONFIG_ENC_KEY`，密文前缀 `enc:v1:`，读取时兼容明文** | 不加密 / 用 KMS | 零外部依赖；前缀让老数据平滑过渡；KMS 在单机部署无必要 |
...
### 阶段 1：可复现 + 安全兜底（4–6 人日）→ 详细计划：`docs/superpowers/plans/2026-09-22-lingshu-migration-phase1-reproducible-and-secure.md`
...
### 阶段 2：测试与可靠性（4–5 人日）→ 详细计划：`docs/superpowers/plans/2026-09-22-lingshu-migration-phase2-tests-and-reliability.md`
...
| 2.6 可观测性 | actuator + prometheus（8081）+ `LingShuMetrics`（AI 成败、派发数、池积压、outbox 积压） | F13 |

**验收：** 前端 `npm test` ≥ 12 用例绿；后端端到端测试在 CI 通过；人为停掉 RocketMQ 后提交任务 → `mq_outbox` 有行，恢复后 2 分钟内自动投递；错误的 AI 配置 → 任务详情可见失败原因；`curl :8081/actuator/prometheus | grep lingshu_` 有业务指标。
...
**核心（阶段 0–3）约 13–18 人日；含可选功能约 18–26 人日。** 阶段 1 是投入产出比最高的一步：完成后灵枢从"看文档相信"变成"任何人能跑"。
...
| # | 风险 | 影响 | 概率 | 缓解 |
|---|---|---|---|---|
| R1 | **生产机内存不足**：LabelHub 现有 ECS 1.6 GB；灵枢栈 JVM(≥512 MB) + MySQL(≈400 MB) + Redis(50 MB) + RocketMQ namesrv+broker(≥768 MB 调优后) + MinIO(≈200 MB) ≈ 2–3 GB | 无法上线 | 高 | 上线前换 ≥ 4 GB 机器；或 MinIO 换成真实 TOS、RocketMQ broker 压到 512 MB、JVM `-Xmx768m` 后勉强跑在 4 GB |
| R2 | **docker.io 镜像拉取失败**（LabelHub 在阿里云 ECS 实测拉不到 `node` 镜像） | 生产无法用 compose | 中 | 生产改系统级安装（灵枢原作者就是 systemd + 原生中间件，被撤回的 `docs/DEPLOY.md` 可从 git `2da1c82^` 恢复参考）；或用阿里云 ACR 镜像加速/自建镜像仓库 |
| R3 | **反推 DDL 与线上真实表结构不一致**（列类型、长度、索引） | 线上库 Flyway baseline 后，后续 `V2+` 迁移可能与真实结构冲突 | 中 | 上线前用 `mysqldump --no-data` 导出线上真实结构与 `V1` 做 diff，以线上为准修正 `V1`；灵枢线上库若能访问，直接以 dump 替换反推版本 |
...
| R5 | **RocketMQ 在 Docker Desktop (Windows) 的 broker 通告地址问题** | 客户端连不上 broker | 中 | `broker.conf` 固定 `brokerIP1=127.0.0.1`（灵枢原作者踩过同一坑，见恢复的部署手册 §一） |
...
| R10 | **授权风险**：灵枢无 LICENSE | 法律/署名争议 | 视情况 | §2 前提 1，取得书面同意后再动手 |
...
1. **并行期。** 阶段 1–3 期间 LabelHub 继续在线；灵枢先部署到新机器（或同机不同端口/域名）。
2. **验收门。** §11 全部通过后，才把域名/入口切到灵枢。
3. **回退。** 切换后 LabelHub 保持可启动 2 周（pm2 `stopped` 状态，不删 release 目录）；回退只需 Nginx 改回 upstream。
4. **数据。** 不迁移 LabelHub 数据；灵枢从种子数据起步（admin + 演示工作空间）。
5. **封存。** 2 周无回退需求后，LabelHub 仓库打 tag `archive/final`，README 标注"已被灵枢取代"。
...
| `lingshu_label_tool` | `LabelToolPO` | `label_tool_code`、`label_tool_name`、`label_tool_type`(1/2)、`label_tool_url`、`label_tool_json_schema` json、`label_tool_page_schema` json、`deleted`、`ext` | `uk_code(label_tool_code)` |
| `lingshu_dataset` | `DatasetPO` | `space_code`、`dataset_name`(≤128)、`dataset_desc`(≤512)、`dataset_type`(1/2/3)、`service_obj_name`(=labelToolCode)、`latest_version_number`、`deleted`、`ext` | `idx_space_name(space_code,dataset_name,deleted)` |
| `lingshu_dataset_version` | `DatasetVersionPO` | `dataset_id`、`version_number`、`version_desc`、`oss_path`、`upload_status`(1/2/3)、`sample_count`、`deleted`、`ext` json | `uk_dataset_version(dataset_id,version_number)`（代码注释确认存在） |
| `lingshu_dataset_sample` | `DatasetSamplePO` | `dataset_version_id`、`biz_id` varchar(64)、`sample_data_json` json、`deleted`、`ext` | `idx_version_biz(dataset_version_id,biz_id)`；`idx_version_id(dataset_version_id,id)` |
...
| 变量 | 必填 | 默认 | 说明 |
|---|---|---|---|
| `LINGSHU_MYSQL_HOST` / `PORT` / `DB` / `USERNAME` | 否 | `127.0.0.1` / `3306` / `lingshu` / `root` | |
| `LINGSHU_MYSQL_PASSWORD` | **是** | 无（缺失即启动失败） | |
| `LINGSHU_REDIS_HOST` / `PORT` | 否 | `127.0.0.1` / `6379` | |
| `LINGSHU_REDIS_PASSWORD` | **是** | 无 | |
| `LINGSHU_ROCKETMQ_NAMESERVER` | 否 | `127.0.0.1:9876` | |
| `LINGSHU_JWT_SECRET` | 首启是 | 无 | 仅当 `sys_config.jwt.secret` 不存在时使用；`openssl rand -base64 48` |
| `LINGSHU_JWT_EXPIRE_SECONDS` | 否 | `86400` | 同上，首启写入 |
| `LINGSHU_ADMIN_INITIAL_PASSWORD` | 首启是 | 无 | 仅当 `admin` 不存在时使用 |
| `LINGSHU_CORS_ALLOWED_ORIGINS` | 否 | `http://localhost:5173` | 逗号分隔 |
| `LINGSHU_OSS_PROVIDER` | 否 | `s3` | `s3` / `tos` |
| `LINGSHU_S3_ENDPOINT` / `REGION` / `ACCESS_KEY` / `SECRET_KEY` / `BUCKET` | provider=s3 时 | 本地 MinIO 值 | endpoint 必须是浏览器可达地址（预签名直传） |
| `LINGSHU_RATE_LIMIT_GLOBAL_PER_MINUTE` | 否 | `600` | 每 IP |
| `LINGSHU_RATE_LIMIT_LOGIN_MAX_FAILURES` / `LOGIN_WINDOW_MINUTES` | 否 | `5` / `15` | |
| `LINGSHU_CONFIG_ENC_KEY` | 阶段 2 起是 | 无 | 32 字节 base64，apiKey 加密 |
| `LINGSHU_SERVER_PORT` / `LINGSHU_MANAGEMENT_PORT` | 否 | `8080` / `8081` | |
